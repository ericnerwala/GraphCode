...
program
  .name('graphcode')
  .description(
    'The graph-native coding agent. Indexes your codebase as a knowledge graph — code structure, git history, docs, features — and puts it in front of the agent before the first token.',
  )
  .version('0.1.0')
...
async function main(): Promise<void> {
  ...
  const argv =
    first && !first.startsWith('-') && !knownCommands.has(first)
      ? process.argv
