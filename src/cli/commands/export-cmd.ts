...
import type { GraphNode, GraphStats } from '../../graph/types.js'
...
function nodeToExport(node: GraphNode): ExportedNode {
  return {
    id: node.id,
    type: node.kind,
...
export function buildExport(store: GraphStore): ExportedGraph {
  const nodeRows = store.raw('SELECT id FROM nodes') as Array<{ id: number }>
  const nodes = nodeRows
    .map((row) => store.getNode(row.id))
    .filter((node): node is GraphNode => node !== null)
    .map(nodeToExport)

  const edgeRows = store.raw('SELECT src, dst, kind, weight FROM edges') as Array<{
    src: number
    dst: number
    kind: string
    weight: number
...
async function runExport(path: string, out: string): Promise<ExportedGraph> {
...
function viewerHtmlPath(): string {
  ...
  if (existsSync(nearPath)) return nearPath
...
export function toVizServerError(err: NodeJS.ErrnoException, port: number): Error {
...
async function runViz(path: string, port: number): Promise<void> {
  const config = loadConfig(path)
  if (!existsSync(config.dbPath)) throw new NotIndexedError(config.root)
  const store = GraphStore.open(config.dbPath)
  ...
  await new Promise<void>((resolvePromise, reject) => {
    server.once('error', (err: NodeJS.ErrnoException) => reject(toVizServerError(err, port)))
