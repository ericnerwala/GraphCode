...
import type { ContextPack, RankCandidate } from './query-types.js'

const SEED_LIMIT = 12
const EXPAND_EDGE_KINDS: readonly EdgeKind[] = ['calls', 'imports', 'contains']
const EXPAND_HOPS = 2
const TIER1_COUNT = 5
const TIER2_COUNT = 8
...
const MIN_HEADER_MARKDOWN =
  '## Graph context (pre-computed from the code graph - verify, then refine; treat listed source as already read)'

export function buildContextPack(store: GraphStore, root: string, task: string, budgetTokens: number): ContextPack {
  ...
  const candidates: RankCandidate[] = files.map((f) => ({ filePath: f.filePath, refs: f.hitCount, direct: f.seed }))
...
function seedSearch(store: GraphStore, task: string): GraphNode[] {
  ...
  for (const node of exact) {
    ...
    if (node.filePath && isTestFile(node.filePath)) continue
    seen.add(node.id)
    nodes.push(node)
  }
  return nodes
...
function expandSeeds(store: GraphStore, seeds: readonly GraphNode[]): GraphNode[] {
  const visited = new Set<number>(seeds.map((n) => n.id))
  const all: GraphNode[] = [...seeds]
  let frontier = seeds
...
function aggregateFiles(nodes: readonly GraphNode[], seeds: readonly GraphNode[]): SeedFile[] {
  ...
  for (const node of nodes) {
    ...
    if (node.kind === 'symbol') acc.symbols.push(node.name)
    acc.nodes.push(node)
...
function renderMarkdown(
  ...
  root: string,
  task: string,
  tier1: readonly RenderFile[],
  tier2: readonly RenderFile[],
  tier3: readonly RenderFile[],
  snippetLines: number,
): string {
  const lines: string[] = []
  ...
  for (const file of tier1) {
    ...
    for (const s of symbolLines) lines.push(s)
    if (snippetLines > 0) {
  ...
  for (const file of tier2) {
    lines.push('')
    lines.push(`#### ${file.filePath}`)
    for (const s of renderSymbolList(file.nodes)) lines.push(s)
  }
...
function renderSymbolList(nodes: readonly GraphNode[]): string[] {
...
function pickMostHitSymbol(nodes: readonly GraphNode[]): GraphNode | null {
  const symbolNodes = nodes.filter((n) => n.kind === 'symbol' && n.filePath && n.startLine !== undefined)
  if (symbolNodes.length === 0) return null
...
function spanOf(node: GraphNode): number {
  if (node.startLine === undefined || node.endLine === undefined) return 0
  return node.endLine - node.startLine
}

function readSnippet(root: string, node: GraphNode, maxLines: number): string | null {
  if (!node.filePath || node.startLine === undefined) return null
  try {
...
function renderGraphContext(store: GraphStore, tier1: readonly RenderFile[]): string[] {
  const lines: string[] = []
  ...
  if (commitLines.length === 0 && docLines.length === 0 && featureLines.length === 0) return []
...
function renderRecentCommits(store: GraphStore, tier1: readonly RenderFile[]): string[] {
  ...
  for (const file of tier1) {
    ...
    for (const c of commits) {
      ...
      if (lines.length >= RECENT_COMMITS_LIMIT) return lines
    }
  }
  return lines
}

function renderDocMentions(store: GraphStore, tier1: readonly RenderFile[]): string[] {
  const lines: string[] = []
  for (const file of tier1) {
    const fileNode = findFileNode(store, file.filePath)
    if (!fileNode) continue
...
function renderFeatureMembership(store: GraphStore, tier1: readonly RenderFile[]): string[] {
  const lines: string[] = []
  for (const file of tier1) {
    const fileNode = findFileNode(store, file.filePath)
    if (!fileNode) continue
