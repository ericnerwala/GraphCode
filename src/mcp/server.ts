...
function resolveOrNull(store: GraphStore, nameOrPath: string): GraphNode | null {
 ...
 */
function callTool(name: ToolName, args: Record<string, unknown>, store: GraphStore, config: GraphcodeConfig): ToolResult {
  switch (name) {
    case 'graph_search': {
      const query = argString(args, 'query')
      if (!query) return errorResult('graph_search requires a "query" string argument')
      const hits = store.search(query, { limit: argNumber(args, 'limit') ?? 20 })
      if (hits.length === 0) return textResult(`no matches for "${query}" — try different terms or graph_explore.`)
      return textResult(stringify(hits))
    }
    case 'graph_explore': {
      const symbols = argStringArray(args, 'symbols')
      if (symbols.length === 0) return errorResult('graph_explore requires a non-empty "symbols" array argument')
      const result = explore(store, config.root, symbols)
      return textResult(stringify(result))
    }
    case 'graph_callers': {
      const symbol = argString(args, 'symbol')
      if (!symbol) return errorResult('graph_callers requires a "symbol" string argument')
      const target = resolveOrNull(store, symbol)
      if (!target) return textResult(`no graph match for "${symbol}" — try graph_search first.`)
      const hits = findCallers(store, target, { depth: argNumber(args, 'depth') ?? 1 })
      return textResult(stringify(hits))
    }
    case 'graph_callees': {
      const symbol = argString(args, 'symbol')
      if (!symbol) return errorResult('graph_callees requires a "symbol" string argument')
      const target = resolveOrNull(store, symbol)
      if (!target) return textResult(`no graph match for "${symbol}" — try graph_search first.`)
    ...
    case 'graph_impact': {
      ...
      if (!targetName) return errorResult('graph_impact requires a "target" string argument')
      const target = resolveOrNull(store, targetName)
      if (!target) return textResult(`no graph match for "${targetName}" — try graph_search first.`)
    ...
    case 'graph_context': {
      ...
      if (!task) return errorResult('graph_context requires a "task" string argument')
...
export async function startMcpServer(store: GraphStore, config: GraphcodeConfig): Promise<void> {
  ...
  server.setRequestHandler(CallToolRequestSchema, (request, _extra): ToolResult => {
