...
export type FindingKind = 'dangling_reference' | 'stale_caller' | 'unresolved_import' | 'unopened_co_change'
 ...
 */
export function verifyEditedFile(
  store: GraphStore,
  config: GraphcodeConfig,
  path: string,
  sync: ReindexResult,
): Finding[] {
  if (!config.postEditVerify || !sync.synced) return []
  try {
...
function checkStaleCallers(sync: ReindexResult): Finding[] {
  if (sync.removedSymbols.length === 0 || sync.priorInboundCallerFiles.length === 0) return []
  const removed = sync.removedSymbols.join(', ')
  return sync.priorInboundCallerFiles
    .filter((f) => f !== sync.path)
    .map((f) => ({
      kind: 'stale_caller' as const,
...
function checkDanglingPendingRefs(sync: ReindexResult): Finding[] {
  if (sync.newlyDanglingRefCount === 0) return []
  return [
    {
      kind: 'dangling_reference',
      severity: 'medium',
...
function checkUnresolvedImports(store: GraphStore, sync: ReindexResult): Finding[] {
  const rows = store.raw(
    `SELECT DISTINCT name FROM pending_refs
     WHERE repo_id = ? AND kind = 'imports'
       AND src_node IN (SELECT id FROM nodes WHERE repo_id = ? AND file_path = ?)`,
    [sync.repoId, sync.repoId, sync.path],
