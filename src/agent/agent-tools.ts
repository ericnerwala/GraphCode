...
import { TOOL_DEFS, type ToolDef } from './tools/tool-defs.js'
import {
  graphSearch,
  graphExplore,
  graphCallers,
...
import { verifyEditedFile, renderFindings, type Finding } from './post-edit-verify.js'

export { TOOL_DEFS, type ToolDef }

const DEFAULT_QUERY_API: QueryApi = realQuery as unknown as QueryApi

/** Callback the loop wires in to record every write for the completion gate. Absent in unit tests. */
export interface SessionStateSink {
  readonly recordWrite: (path: string, sync: ReindexResult, findings: readonly Finding[]) => void
}
...
export interface DispatchContext {
  ...
  /** Wired by loop.ts only; dispatchTool degrades gracefully when absent. */
  readonly sessionState?: SessionStateSink
}

/** The tools that mutate files. The loop serializes these within a turn (see loop.ts). */
export const MUTATING_TOOLS: ReadonlySet<string> = new Set(['write_file', 'edit_file'])
...
export async function dispatchTool(name: string, input: unknown, ctx: DispatchContext): Promise<string> {
 ...
 */
async function finishEditDispatch(
  ctx: DispatchContext,
  path: string,
  result: string,
  guardAdvisory: string,
): Promise<string> {
  if (result.startsWith(ERROR_PREFIX)) return result
  ...
  const findings: Finding[] =
    ctx.config.postEditVerify && sync.synced ? safeVerify(ctx, path, sync) : []
 ...
 */
function computeEditGuardAdvisory(api: QueryApi, ctx: DispatchContext, path: string): string {
  if (!ctx.config.liveGraphSync || !ctx.config.editGuard.enabled) return ''
  try {
...
async function safeReindex(ctx: DispatchContext, path: string): Promise<ReindexResult> {
  try {
...
function safeVerify(ctx: DispatchContext, path: string, sync: ReindexResult): Finding[] {
  try {
...
function renderSyncNote(sync: ReindexResult): string {
  const parts: string[] = []
  if (sync.addedSymbols.length > 0) parts.push(`+${sync.addedSymbols.length} symbol(s): ${sync.addedSymbols.join(', ')}`)
  if (sync.removedSymbols.length > 0) parts.push(`-${sync.removedSymbols.length} symbol(s): ${sync.removedSymbols.join(', ')}`)
