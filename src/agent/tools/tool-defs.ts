...
export const TOOL_DEFS: readonly ToolDef[] = [
  {
    name: 'graph_search',
    description:
      'Full-text search over the code knowledge graph. Returns matching symbols (file, line, kind) ranked by relevance. Use to locate exact symbol names before graph_explore/graph_callers/graph_callees/graph_impact.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'graph_explore',
    description:
      "PRIMARY graph tool for flow/trace questions. Given a bag of symbol names (functions, methods, classes), connects and returns the call flow among them. Use FIRST for any 'how does X reach Y' / trace / data-flow question. Treat the returned source as already read.",
    input_schema: {
      type: 'object',
      properties: {
        symbols: {
  ...
  },
  {
    name: 'graph_impact',
    description:
      'Compute the impact radius (blast radius) of a symbol - the transitive set of code affected if it changes. Use BEFORE editing a symbol to scope the change.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'graph_context',
    description:
      'Build a ranked context pack for a task description: the most relevant files/symbols in the repo for this task, straight from the graph. Use when starting work on a broad task and the turn-0 pack is insufficient or stale.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'edit_file',
    description:
      'Edit a file via exact string replacement. old_string must match exactly one location unless replace_all is set.',
    input_schema: {
      type: 'object',
      properties: {
        path: { type: 'string', description: 'Repo-root-relative file path.' },
  ...
  },
  {
    name: 'bash',
    description:
      'Run a shell command in the repository root. Use to run tests/builds and verify edits. 120s default timeout; output is clamped.',
    input_schema: {
      type: 'object',
      properties: {
