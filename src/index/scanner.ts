...
export interface ScannedFile {
  readonly path: string // relative to repo root, forward-slash separated
...
export interface ScanResult {
  readonly codeFiles: readonly ScannedFile[]
  readonly docFiles: readonly ScannedFile[]
}
...
function loadRootIgnore(root: string): Ignore {
  const ig = ignore()
  try {
...
export async function scanRepo(
  root: string,
  options: { extraIgnore?: readonly string[] } = {},
): Promise<ScanResult> {
  const ig = loadRootIgnore(root)
  if (options.extraIgnore && options.extraIgnore.length > 0) ig.add([...options.extraIgnore])

  const codeFiles: ScannedFile[] = []
  ...
  async function walk(dirAbs: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (BUILTIN_EXCLUDE_DIRS.has(entry.name)) continue
      ...
      if (ig.ignores(relPath)) continue
      if (isMinifiedJs(entry.name)) continue
      ...
      const file: ScannedFile = { path: relPath, absPath, size: stats.size, mtime: Math.trunc(stats.mtimeMs) }
      if (ext === '.md' || ext === '.mdx') {
        docFiles.push(file)
