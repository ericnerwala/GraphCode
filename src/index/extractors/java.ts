 ...
 */
function typeRootName(node: Node): string | undefined {
...
function handleMethod(node: Node, className: string, ctx: WalkContext): void {
  ...
  if (params) {
    for (const param of namedKids(params)) {
      ...
      if (typeName) ctx.refs.push({ fromSymbol: name, name: typeName, kind: 'references' })
    }
  }
...
function handleField(node: Node, className: string, ctx: WalkContext): void {
  ...
  for (const declarator of declarators) {
    ...
    if (fieldTypeName) ctx.refs.push({ fromSymbol: className, name: fieldTypeName, kind: 'references' })
  }
}
