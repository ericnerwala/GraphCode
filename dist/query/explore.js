...
function shortestPath(store, from, to, maxHops) {
    if (from.id === to.id)
        return [];
    const visited = new Set([from.id]);
    let frontier = [{ node: from, edges: [] }];
    for (let hop = 0; hop < maxHops; hop++) {
        ...
        for (const frame of frontier) {
            ...
            for (const neighbor of neighbors) {
                ...
                const edge = neighbor.direction === 'out'
...
function inlineSource(root, node, maxLines) {
