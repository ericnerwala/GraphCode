...
function walkExpressionForRefs(node, enclosing, ctx) {
    ...
    for (const call of descendantsOf(node, ['method_invocation'])) {
        ...
        if (nameNode)
            ctx.refs.push({ fromSymbol: enclosing, name: nameNode.text, kind: 'calls' });
    }
...
function handleTypeDecl(node, kind, ctx, parentName) {
...
function handleMethod(node, className, ctx) {
    const nameNode = node.childForFieldName('name');
    if (!nameNode)
        return;
    const name = nameNode.text;
