...
function receiverTypeName(receiverNode) {
...
function referenceParamAndResultTypes(paramsNode, resultNode, enclosing, ctx) {
...
function handleFuncDecl(node, ctx) {
...
function handleTypeSpec(node, ctx) {
    const nameNode = node.childForFieldName('name');
        ...
        if (fieldList) {
            for (const field of namedKids(fieldList)) {
                ...
                if (!fieldNameNode) {
                    ...
                    if (typeRef)
                        ctx.refs.push({ fromSymbol: name, name: identifierRoot(typeRef), kind: 'extends' });
                    continue;
                }
...
function handleImportSpec(node, ctx) {
...
function handleTopLevelVarConst(node, kind, ctx) {
    for (const spec of namedKids(node)) {
        if (spec.type !== 'var_spec' && spec.type !== 'const_spec')
            continue;
        const nameNodes = spec.childrenForFieldName('name').filter((n) => n !== null);
            ...
            case 'type_declaration':
                for (const spec of namedKids(child)) {
                    if (spec.type === 'type_spec')
                        handleTypeSpec(spec, ctx);
                }
                break;
            case 'import_declaration':
                for (const spec of namedKids(child)) {
                    if (spec.type === 'import_spec')
                        handleImportSpec(spec, ctx);
                    else if (spec.type === 'import_spec_list') {
                        for (const inner of namedKids(spec))
                            handleImportSpec(inner, ctx);
                    }
                }
                break;
...
export function extractGo(tree) {
