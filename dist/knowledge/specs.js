...
function classifyDoc(path) {
    const base = (path.split('/').at(-1) ?? path).toLowerCase();
    if (base.startsWith('readme'))
        return 'readme';
    if (base.startsWith('changelog') || base.startsWith('history'))
        return 'changelog';
    if (/adr[-_]?\d/.test(base) || path.toLowerCase().includes('/adr'))
        return 'adr';
    if (base.includes('spec') || path.toLowerCase().includes('/specs/') || path.toLowerCase().includes('/rfcs/'))
        return 'spec';
    return 'doc';
}
...
function extractBacktickSpans(body) {
 ...
 */
export function ingestDocs(store, repo, config, docFiles, options = {}) {
    ...
    store.transaction(() => {
        for (const docFile of files) {
            ...
            for (const candidatePath of extractPathMentions(body)) {
                if (mentionsForDoc >= MAX_MENTIONS_PER_DOC)
                    break;
                if (candidatePath === docFile.path)
                    continue;
            ...
            for (const symbolName of extractBacktickSpans(body)) {
                ...
                if (matches.length === 0 || matches.length > MAX_AMBIGUOUS_MATCHES)
                    continue;
