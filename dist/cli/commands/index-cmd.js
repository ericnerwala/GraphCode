 ...
 */
async function runSyncPipeline(store, config, options = {}) {
    ...
    if (!repo)
        throw new Error(`repo not found after indexing: ${config.root}`);
    if (!options.noGit) {
 ...
 */
export async function ensureIndexed(config, opts = {}) {
...
function printSummary(summary) {
    print(`indexed ${summary.filesIndexed} files (${summary.filesDeleted} removed), ${summary.symbols} symbols, ${summary.edges} edges` +
...
async function runIndex(path, options) {
...
async function runWorkspaceIndex(path) {
    const config = loadConfig(path);
...
export function registerIndexCommands(program) {
    program
        .command('index')
