...
function emit(json, data, humanize) {
    if (json) {
...
function resolveOrThrow(store, nameOrPath) {
...
function printSearchHits(hits) {
    if (hits.length === 0) {
        print('no matches');
        return;
    }
    for (const hit of hits)
        print(`${hit.score.toFixed(2)}  ${describeNode(hit.node)}`);
}
function printTraversalHits(hits) {
    if (hits.length === 0) {
        print('none');
        return;
    }
...
function printImpactResult(result) {
        ...
        if (options.workspace && config.workspaceRepos.length > 0) {
            ...
            try {
                ...
                emit(options.json, hits, () => {
                    ...
                    for (const hit of hits)
                        print(`${hit.score.toFixed(2)}  [${hit.repo}] ${describeNode(hit.node)}`);
                });
            }
            finally {
