...
const SEVERITY_FLOOR = { high: 2, medium: 1, low: 0 };
/** Decide whether to hold the turn open with a follow-up. Read-only; never throws. */
export function runCompletionGate(store, config, state) {
    if (!config.completionGateEnabled)
        return { shouldGate: false };
    if (state.gateIterations >= config.completionGateMaxIterations)
        return { shouldGate: false };
    if (state.writtenFiles.size === 0)
        return { shouldGate: false };
    try {
...
function checkUnopenedCoChanges(store, state) {
    const findings = [];
    const seen = new Set();
    for (const [path, record] of state.writtenFiles) {
        ...
        for (const n of neighbors) {
            ...
            if (!neighborPath || state.writtenFiles.has(neighborPath) || seen.has(neighborPath))
                continue;
...
function dedupeByMessage(findings) {
    const byMessage = new Map();
    for (const f of findings)
        if (!byMessage.has(f.message))
