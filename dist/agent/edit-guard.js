...
export function buildEditGuardAdvisory(api, store, root, repoId, path, options) {
    if (!options.enabled)
        return '';
    try {
        ...
        if (symbols.length > options.maxSymbolsPerFile)
            return '';
        const { files, coChanges } = aggregateImpact(api, store, root, path, symbols, options);
        if (files.length < options.minImpactedFiles)
            return '';
...
function aggregateImpact(api, store, root, path, symbols, options) {
    const merged = new Map();
    const coChangeMerged = new Map();
...
function renderAdvisory(files, coChanges, topFiles, topCoChanges) {
    ...
    for (const f of shown)
        lines.push(`  - [${f.tier}] ${f.filePath} (via ${[...f.viaSymbols].join(', ')})`);
    ...
    if (ccShown.length > 0) {
        ...
        for (const c of ccShown)
            lines.push(`  - ${c.filePath} with ${c.withFile} (weight ${c.weight.toFixed(2)})`);
    }
    return lines.join('\n');
}
