...
export function runBash(root, input) {
    ...
    const parts = [];
    if (result.signal)
        parts.push(`killed by signal ${result.signal} (likely timeout after ${timeout}ms)`);
    ...
    if (result.stderr)
        parts.push(`stderr:\n${result.stderr}`);
    if (timedOut)
        parts.push(`[timed out after ${timeout}ms; output above was captured before timeout]`);
